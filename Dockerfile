...
ARG PKGURL=https://dl.ui.com/config/unifi7.3.83/unifi_sysvinit_all.deb

ENV BASEDIR=/usr/lib/unifi \
    DATADIR=/config/unifidata \
    LOGDIR=/config/unifilog \
    CERTDIR=/config/unificert \
    RUNDIR=/config/unifirun \
...
RUN mkdir -p /usr/unifi \
     /usr/local/config/unifiinit.d \
     /usr/config/unifiinit.d \
     /usr/local/docker
COPY docker-entrypoint.sh /usr/local/bin/
COPY docker-healthcheck.sh /usr/local/bin/
COPY docker-build.sh /usr/local/bin/
COPY functions /usr/config/unififunctions
COPY import_cert /usr/config/unifiinit.d/
COPY pre_build /usr/local/docker/pre_build
RUN chmod +x /usr/local/bin/docker-entrypoint.sh \
 && chmod +x /usr/config/unifiinit.d/import_cert \
...
COPY hotfixes /usr/local/config/unifihotfixes

RUN chmod +x /usr/local/config/unifihotfixes/* && run-parts /usr/local/config/unifihotfixes

VOLUME ["/unifi", "${RUNDIR}"]

EXPOSE 6789/tcp 8080/tcp 8443/tcp 8880/tcp 8843/tcp 3478/udp 10001/udp

WORKDIR /config/unifi
...
#ENTRYPOINT ["/usr/bin/java", "-Xmx${JVM_MAX_HEAP_SIZE}", "-jar", "/usr/lib/config/unifilib/ace.jar"]
